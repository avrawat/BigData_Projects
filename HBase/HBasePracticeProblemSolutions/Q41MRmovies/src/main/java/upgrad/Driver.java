...
public class Driver extends Configured implements Tool {
	...
	public int run(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
		...
		// define job ojectConnection
		Job job2 = new Job(conf, "FinalSummary");
		...
		/******************** Creating Table ******************/
		System.out.println("Creating table named " + targetTable);
		HTableDescriptor htable1 = new HTableDescriptor(TableName.valueOf(targetTable));
		htable1.addFamily(new HColumnDescriptor("R"));
		admin.createTable(htable1);
		System.out.println("table '" + targetTable + "' is Created");
		/******************************************************/
		...
		Logger.getRootLogger().setLevel(Level.INFO);
		TableMapReduceUtil.initTableMapperJob(sourceTable, scan1, MyMapper2.class, IntWritable.class, Text.class, job2);
		TableMapReduceUtil.initTableReducerJob(targetTable, MyReducer2.class, job2);
		
		
		
		job2.setNumReduceTasks(1); // by default it is one, incase of large cluster increse it
